...
const Nav = () => {
  return (
    <div className="fixed h-[12vh] z-[10] bg-[#1c1b22] w-full">
      <div className="flex items-center h-full justify-between mx-auto w-[95%] sm:w-[90%] xl:w-[80%]">
        ...
        <div className="flex items-center space-x-10">
          ...
          <div className="flex items-center space-x-4">
            <button className="px-4 py-2 bg-green-500 rounded-3xl sm:font-semibold font-light hover:bg-green-600 transition-all duration-200">
              Hire Me
            </button>
      ...
      <div className="fixed top-0 left-0 w-[70%] sm:w-[50%] h-full bg-[#1c1b22] z-[100] p-6">
        {/*Logo and closeMenu*/}
        ...
        <div className="flex flex-col justify-center items-start space-y-6 py-10">
          {navLinks.map((nav) => (
            <Link key={nav.id} href={nav.url}>
              <p className="nav_label text-[18px] border-white border-b-[1px] hover:border-none">
                {nav.label}
              </p>
            </Link>
          ))}
